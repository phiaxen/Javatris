...
/**
 * THIS CLASS IS A CONTROLLER Controller is a class that handles all user
 * inputs.
 * 
		...
		if ((e.getID() == KeyEvent.KEY_PRESSED)) {
			...
			if (gameEngine.running()) {
				if ((key == KeyEvent.VK_RIGHT)) {
					if (!(gameEngine.getCurrentShape().getX()
							+ gameEngine.getCurrentShape().getCoords()[0].length == 10)) {
